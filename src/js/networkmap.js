...
;(function() {
  ...
  DragHandler.prototype.drag = function(e){
    ...
    if (typeof c == 'function') {
      ...
      var coord = c.call(this.el, x, y, this.m) // CONSTRAINT FONCTION
...
networkMap.widget.ErrorInfo = function(label, value, options){
	this.options = {
		class: 'nm-input-text',
		type: 'text'
	};
...
networkMap.extend(networkMap.widget.ErrorInfo, networkMap.Observable);
networkMap.extend(networkMap.widget.ErrorInfo, networkMap.Options);
networkMap.extend(networkMap.widget.ErrorInfo, {	
	createElements: function(label, value){
		var wrapper = this.wrapper = document.createElement('div');
		wrapper.classList.add(this.options.class);

		var input = this.input = document.createElement('span');
...
networkMap.extend(networkMap.widget.ColorInput, {
	...
	createElements: function(label, value){
		...
		this.input.className= "color_selector";
		this.input.addEventListener('change', function(e){
			this.fireEvent('change', [e, this]);
		}.bind(this));
...
networkMap.extend(networkMap.widget.Accordion, {	
	...
	add: function(label, options,start_open=1){
		var item = document.createElement('div');
		item.classList.add('nm-accordion-group');
		
		if(start_open)
			item.classList.add('nm-accordion-open');
...
networkMap.extend(networkMap.widget.GridInput, {
	createElements: function(label, value){
		...
		this.check.addEventListener('change', function(e){
			...
			this.xy.input.disabled = !this.check.checked;
			e.value = this.value();
			this.fireEvent('change', [e]);
		}.bind(this));
		this.check.addEventListener('onblur', function(e){
			this.x.input.disabled = !this.check.checked;
			this.y.input.disabled = !this.check.checked;
			e.value = this.value();
			this.fireEvent('change', [e]);
		...
		this.xy = this.$createInputs('xy', value.grid.xy, value.enabled);
		this.wrapper.appendChild(this.label);
		this.wrapper.appendChild(this.check);
		this.wrapper.appendChild(this.xy.input);
	...
	$createInputs: function(label, value, enabled){
		...
		els.input.addEventListener('mouseout', function(e){
			e.value = this.value();
			this.fireEvent('change', [e, this]);
		}.bind(this));
		return els;
	...
	value: function(){
		return {
			...
			grid: {
				xy: parseInt(this.xy.input.value, 10),
			}
		};
	},
	...
	if (name === 'click'){
		...
		networkMap.events.mouseover = function(e, link){
			...
			*/
			...
			var size = el.getSize();
			var bboxclientX = e.target.getBoundingClientRect();
 ...
 * Flat5 colormap
 */
networkMap.colormap.flat10 = {
	translate: function(value){
		var map = networkMap.colormap.flat10;
		if (!value && value !== 0)
			return map.nodata;
	
		if (value === 0)	return map.map[0];
		if (value < 10)	return map.map[1];
		if (value < 20)	return map.map[2];
		if (value < 30)	return map.map[3];
		if (value < 40)	return map.map[4];
		if (value < 50)	return map.map[5];
		if (value < 60)	return map.map[6];
		if (value < 70)	return map.map[7];
		if (value < 80)	return map.map[8];
		if (value < 90)	return map.map[9];
...
networkMap.extend(networkMap.SettingsManager, {
	...
	createMenu: function(){
		...
		var LnkButton = this.btnAdd = document.createElement('button');
		LnkButton.textContent = 'Link';
		LnkButton.classList.add('btn', 'btn-success');
		LnkButton.addEventListener('click', function() {			lnk_but(event);		  }) ;
...
networkMap.extend(networkMap.renderer.link.UtilizationLabel, {
	...
	render: function(value,link){return this.state.render.call(this, value,link); },
...
networkMap.extend(networkMap.Graph, {
	...
	getSettingsWidget: function(){
		...
		accordionGroup = container.add('Node Defaults',0,0);		
		...
		accordionGroup = container.add('Link Defaults',0,0);
	...
	//	this.triggerEvent('resize', this);
		...
		if(map.properties.properties.backgroundColor)
			document.body.style.backgroundColor=map.properties.properties.backgroundColor
		...
		request.onload = function() {
			...
			if (request.status >= 200 && request.status < 400){
				...
				if (data.status === 'need_password'){
					new networkMap.widget.Modal().alert("Please enter the password to save changes<br/><br/><center><input id='the_password' type='password'></center><br/></input>", {title: 'Error'});
					
					var btn = document.querySelector('.modal-footer').innerHTML = "<a href='#' class='btn' onclick='send_password()'>Ok</a>"
	...
	removeLink: function(link){
		...
		var id_delete = link2ID(link)
		log(id_delete)
		if(id_delete === false)
			alert("error cant find link id")
		delete(map.links[id_delete])

		map.links = map.links.filter(function(val){return val}); // re-index array
...
networkMap.extend(networkMap.Graph.Module.Settings, {
	toElement: function(nodeProperties, linkProperties, graphProperties){
		...
		accordionGroup.appendChild(new networkMap.widget.ColorInput('Background',
		graphProperties.get('backgroundColor') )
	.addEvent('change', function(e){
			var color = this.el.items[0].querySelector('.color_selector').value
...
networkMap.extend(networkMap.Node, {
	 ...
	 * @return {undefined}
	 */
	_clickhandler: function(e){ // CLICK ON NODE
	...
	draw: function(){
		...
		svg.on('dragend', function(event){ // dragend for NODE
			...
			this.properties.set({
				x: this.svg.tbox().x-diffX,
				y: this.svg.tbox().y-diffY
			});
...
networkMap.Node.registerRenderer('image', function(node, svg) {
	// create the label first to get size
	var label = svg.text(node.options.name)
		.font({
			family:   node.options.fontFamily,
		...
		if (this.properties.get('requestUrl')) {
			this.getLink().registerUpdateEvent(
				...
				function(response){
					...
					var nodeErrorID = this.getSubLink().node.options.id
					
					
					if(this.getLink().options.nodeA.id==nodeErrorID)
						{
						this.getLink().options.nodeA.error=response.error
						}
					else
						{
						this.getLink().options.nodeB.error=response.error
						}
...
networkMap.extend(networkMap.SubLink, {
	...
	setUtilizationLabel: function(value){
		...
		this.utilizationLabel.render(value,this.getLink());
...
networkMap.extend(networkMap.Link, {
	...
	_setupSVG: function(options){
		...
		// CHECK request from link (SNMP)
		
		if (this.properties.get('request_type')=="SNMP" && this.properties.get('requestUrl')) {
	...
	edgePoints: function(){
		...
		if(document.getElementsByClassName('svg-pan-zoom_viewport')[0])
			{
			var viewportTRansform = document.getElementsByClassName('svg-pan-zoom_viewport')[0].style.transform
			var velem = viewportTRansform.split(',')
			diffX=parseInt(velem[4])
			diffY=parseInt(velem[5])
			}
		...
		this.$edgePoints = {
			...
			nodeA: {
				point: new SVG.math.Point(edgePointA.x-diffX, edgePointA.y-diffY),
				pointer: edgePointerA,
				direction: dirA
			},
			nodeB: {
				point: new SVG.math.Point(edgePointB.x-diffX, edgePointB.y-diffY),
			...
			networkMap.each(linkTemplate, function(option, key){
				...
				if (['id'].some(function(item){ return item == key;})){
					...
					if (option.type === 'number'){
						...
						if(link.subLinks[node].primaryLink.properties.get(key))
						{
							accordionGroup.appendChild(
								new networkMap.widget.ErrorInfo(
									option.label, 
									link.subLinks[node].primaryLink.properties.get(key), 
									option
								)
			...
			show: function(bbox){
				...
				var edgeHandle = this.edgeHandle = this.svg.circle(this.size)
					...
					.draggable(function(x, y){
						...
						x = x < (bxok - radius) ? bxok - radius : x ;
						x = x > (bxok + this.bbox.width - radius) ? bxok + this.bbox.width - radius : x ;
						y = y < (byok - radius) ? byok - radius : y ;
