...
networkMap.extend(networkMap.SettingsManager, {
	...
	createMenu: function(){
		...
		saveButton.addEventListener('click', save_map);
...
networkMap.extend(networkMap.Node, {
	...
	draw: function(){
		...
		svg.on('dragstart', function(event){
			...
			if(this.tspan) this.tspan.remove()// hide label
		...
		svg.on('dragend', function(event){ // dragend for NODE
			...
			add_label(this,0) // replace label
...
networkMap.Node.registerRenderer('rect', function(node, svg) {
	...
	if(node.tspan) node.tspan.remove()// hide label
	...
	if(node.options.bgImage)
		...
		rect.node.addEventListener('load', function() { add_label(node,0); });
		}
	else
		{
